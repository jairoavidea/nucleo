...
Write-Host "Installing Núcleo 1.0..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "Núcleo 1.0.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "Núcleo 1.0"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named Núcleo 1.0. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching Núcleo 1.0..."
